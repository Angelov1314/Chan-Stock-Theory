...
使用方法：
  python chan_annotator.py --csv your_data.csv --out out.png
...
# 注：缠论原始包含较复杂，这里以稳健方式减少毛刺；可按你口径替换
def resolve_inclusion(df: pd.DataFrame) -> pd.DataFrame:
    # 简化策略：若相邻K线出现包含，则将被包含K线的高低吸收到包含K线
    # 实操中常用于更顺滑的分型识别。这里不改变原K线数量，仅平滑高低点。
...
def plot_candles_with_annotations(df: pd.DataFrame,
    ...
    for f in frs:
        if f.kind == "top":
            ax.scatter(f.idx, f.price, marker="^", s=60)
        else:
            ax.scatter(f.idx, f.price, marker="v", s=60)

    # 笔（连线）
    for s in strokes:
        ax.plot([s.start_idx, s.end_idx], [s.start_price, s.end_price], linewidth=2)

    # 线段（更粗）
    for seg in segs:
        ax.plot([seg.start_idx, seg.end_idx], [seg.low, seg.low], linestyle="--", linewidth=1.5)
        ax.plot([seg.start_idx, seg.end_idx], [seg.high, seg.high], linestyle="--", linewidth=1.5)
    ...
    for idx, kind in divs:
        ...
        ax.text(idx, y, txt, fontsize=9, ha="center", va="bottom")
